...
def parse_args():
    ...
    parser.add_argument('--optical', help='Optical Flow',
                        default=False, type=bool) 
...
def main():
    ...
    for epoch in range(last_epoch, config.TRAIN.END_EPOCH):
        ...
        if(args.test):
            ...
            psnr_list = inference(config, test_loader, model,args,quit=True)
            ...
            if(auc>AUC_memory):
                ...
                torch.save(model.module.state_dict(), os.path.join(final_output_dir,
                AUC_memory=auc                                               'best_AUC.pth'.format(epoch + 1)))
...
def train(config, train_loader, model, loss_functions, optimizer, epoch, logger,args):
    ...
    for i, data in enumerate(train_loader):
        ...
        if(args.optical):
          optical=data['optical_flow']
          output = model(inputs,optical)
        else:
          output = model(inputs)
...
def train_pseudo(config, train_loader,train_loader_jump, model, loss_functions, optimizer, epoch, logger,pseudolossepoch, pseudolosscounter,args):
    ...
    for i, (data,data_jump) in enumerate(zip(train_loader,train_loader_jump)):
        ...
        if(args.optical):
          optical=data['optical_flow']
          optical_jump=data_jump['optical_flow']
          optical_batch=train_util.To_Batch(optical,config.TRAIN.BATCH_SIZE_PER_GPU * len(list(config.GPUS)),config.MODEL.ENCODED_FRAMES)
          optical_batch_jump=train_util.To_Batch(optical_jump,config.TRAIN.BATCH_SIZE_PER_GPU * len(list(config.GPUS)),config.MODEL.ENCODED_FRAMES)
        # images 
        inputs, target = train_util.decode_input(input=data, train=True)
       ...
       # target_batch_jump=To_Batch(target_jump,config.TRAIN.BATCH_SIZE_PER_GPU * len(list(config.GPUS)),config.MODEL.ENCODED_FRAMES)
        ...
        for b in range(config.TRAIN.BATCH_SIZE_PER_GPU * len(list(config.GPUS))):
          ...
          if pseudo_anomaly_jump:
            ...
            if(args.optical):
              optical_batch[b] = optical_batch_jump[b]
        ...
        if(args.optical):
          optical=train_util.To_Frame(optical_batch,config.TRAIN.BATCH_SIZE_PER_GPU * len(list(config.GPUS)),config.MODEL.ENCODED_FRAMES)
          output = model(inputs,optical)
        else:
          output = model(inputs)
      ...
      # loss calculation 
        modified_inte_loss ,modified_grad_loss,modified_msssim_loss,modified_l2_loss  = [],[],[],[]
        for b in range(config.TRAIN.BATCH_SIZE_PER_GPU * len(list(config.GPUS))):
          ...
          if jump_pseudo_stat[b]:
            ...
            pseud=inte_loss_b+grad_loss_b+msssim_loss_b+l2_loss_b
            pseud /= -4
        ...
        if (i + 1) % config.PRINT_FREQ == 0:
            msg = 'Epoch: [{0}][{1}/{2}]\t' \
                  'Lr {lr:.6f}\t' \
                  '[inte {inte:.5f} + grad {grad:.4f} + msssim {msssim:.4f} + L2 {l2:.4f} + pseudo {pseudo:.4f} ]\t' \
                  'PSNR {psnr:.2f}'.format(epoch+1, i+1, len(train_loader),
                                             lr=cur_lr,
                                             inte=inte_loss, grad=grad_loss, msssim=msssim_loss, l2=l2_loss,
